...
// Pastikan semua key selalu ada dan string (bukan NULL) agar aman untuk htmlspecialchars:
$def_detail = [
...
if (!$detail) {
    ...
    foreach ($def_detail as $k => $defv) {
        if (!isset($detail[$k]) || $detail[$k] === null) $detail[$k] = $defv;
    }
}
...
<body>
  <div class="container-detail">
    ...
    <section class="info-box">
      ...
      // Tambahan Mobile Maps
      if ($detail['link_map_mobile']) {
        echo '<div class="mt-3">';
        echo '<strong>Link Google Maps untuk Mobile:</strong> ';
        echo '<a href="' . htmlspecialchars($detail['link_map_mobile']) . '" target="_blank" rel="noopener" class="btn btn-success ms-2 mt-2"><i class="bi bi-geo-alt"></i> Buka di Google Maps (HP)</a>';
        echo '</div>';
      }
      ?>
    </section>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
      <form class="modal-content" id="formDetailTrip">
        ...
        <div class="modal-body">
          ...
          <div class="mb-3">
            <label for="link_map_mobile" class="form-label">
              <i class="bi bi-link-45deg"></i> Link Google Maps untuk Mobile (Share Link Google Maps)
            </label>
            <input type="url" class="form-control" id="link_map_mobile" name="link_map_mobile"
              value="<?= htmlspecialchars($detail['link_map_mobile'] ?? '') ?>"
              placeholder="Link Google Maps Mobile (misal: https://maps.app.goo.gl/xyz atau https://maps.google.com/?q=...)" />
            <small class="form-text text-muted">Link ini akan digunakan aplikasi mobile untuk membuka Google Maps.</small>
          </div>
          <div class="mb-4">
