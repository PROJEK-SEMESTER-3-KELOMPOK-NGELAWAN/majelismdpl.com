...
if (!isset($_SESSION['id_user']) || empty($_SESSION['id_user'])) {
    header('Location: ' . getPageUrl('index.php') . '?error=unauthorized');
    exit;
}
...
if (isset($_FILES["admin_foto_profil"]) && $_FILES["admin_foto_profil"]["error"] == 0) {
    ...
    if ($file["size"] > $max_file_size) {
        header('Location: ' . getPageUrl('admin/index.php') . '?error=access_denied&message=' . urlencode('Ukuran file terlalu besar. Maksimal 2MB.'));
        exit;
    }

    $allowed_types = ['image/jpeg', 'image/png', 'image/gif'];
    if (!in_array($file["type"], $allowed_types)) {
        header('Location: ' . getPageUrl('admin/index.php') . '?error=access_denied&message=' . urlencode('Hanya file JPG, PNG, dan GIF yang diizinkan.'));
        exit;
    }
    ...
    if (move_uploaded_file($file["tmp_name"], $target_file)) {
        ...
        if ($stmt_update->execute()) {
            ...
            $stmt_update->close();
            $conn->close();
            header('Location: ' . getPageUrl('admin/index.php') . '?success=photo_updated'); // Tambahkan pesan sukses
            exit;
        } else {
            ...
            $stmt_update->close();
            $conn->close();
            header('Location: ' . getPageUrl('admin/index.php') . '?error=access_denied&message=' . urlencode('Gagal menyimpan jalur foto ke database.'));
            exit;
        }
    } else {
        header('Location: ' . getPageUrl('admin/index.php') . '?error=access_denied&message=' . urlencode('Gagal mengupload file ke server.'));
        exit;
    }
} else {
    // Error upload file lainnya
    header('Location: ' . getPageUrl('admin/index.php') . '?error=access_denied&message=' . urlencode('Tidak ada file yang dipilih atau terjadi error saat upload.'));
    exit;
}
